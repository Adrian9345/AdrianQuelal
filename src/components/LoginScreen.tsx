...
import { loginWithEmail, googleSignIn, facebookSignIn } from '../firebaseAuth';
...
export default function LoginScreen({ onLogin, onNavigateRegister, onNavigateForgotPassword }: { onLogin: () => void, onNavigateRegister: () => void, onNavigateForgotPassword: () => void }) {
  ...
  const handleFacebookSignIn = async () => {
    ...
    try {
      ...
      console.error("Facebook sign in error details:", err);
      const msg = err?.message || '';
      const code = err?.code || '';
      if (msg.includes('unauthorized-domain') || code.includes('unauthorized-domain') || msg.includes('dominio no autorizado')) {
        setError('Este dominio no está autorizado en tu consola de Firebase.');
        ...
        setError(msg || 'Error al iniciar sesión con Facebook');
      }
    } finally {
      setLoading(false);
    }
  ...
  return (
    ...
    >
      ...
      {showBlockedHelper && (
        <div className="fixed inset-0 bg-[#1B1C19]/90 backdrop-blur-md z-50 flex items-center justify-center p-4 overflow-y-auto">
          <div className="bg-[#2D2E2A] text-[#F0EEE9] border border-[#f39233] rounded-3xl max-w-lg w-full p-6 shadow-2xl relative my-8">
            ...
            <div className="space-y-4 mb-6 text-xs text-[#F0EEE9]/90 font-medium">
              <div className="bg-black/20 p-3.5 rounded-2xl border border-white/5 space-y-2">
                ...
                <p className="leading-relaxed">Cualquier persona que intente iniciar sesión con Google (como <strong className="text-white">blackmc4523@gmail.com</strong> o tu propio correo) debe estar registrada como usuario de prueba en tu consola.</p>
                ...
                <div className="space-y-1.5 pl-3 border-l-2 border-yellow-400/40 mt-2">
                  ...
                  <p>2. Selecciona tu proyecto <span className="font-mono text-white bg-black/40 px-1 py-0.5 rounded text-[10px] font-bold">raigal-app</span> desde el selector superior.</p>
